...
async function createPrescription(event) {
  ...
  if (scheduleTime.value || reminderType.value) {
    successText += ` Schedule time (${scheduleTime.value || "not saved yet"}) and reminder type (${reminderType.value}) are kept ready for your next APIs.`;
  }
...
function clearForm() {
  ...
  voiceStatus.textContent = "Speak medicine name, dosage, frequency or instructions.";
}
